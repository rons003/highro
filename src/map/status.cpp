...
#define set_sc_with_vfx(skill, sc, icon, flag) \
	set_sc((skill), (sc), (icon), (flag));     \
	if ((icon) < EFST_MAX)                     \
...
bool status_check_skilluse(struct block_list *src, struct block_list *target, uint16 skill_id, int flag)
{
	...
	case ALL_ODINS_POWER:
		...
		if (src && map_getcell(src->m, src->x, src->y, CELL_CHKLANDPROTECTOR) && !status_has_mode(status, MD_STATUS_IMMUNE) && (src->type != BL_PC || ((TBL_PC *)src)->skillitem != skill_id))
			return false;
		break;
	case SC_MANHOLE:
	...
	if (sc && sc->count)
	{
		...
		if (skill_id != RK_REFRESH && skill_id != SU_GROOMING && sc->opt1 && sc->opt1 != OPT1_BURNING && skill_id != SR_GENTLETOUCH_CURE)
		{				   // Stuned/Frozen/etc
		...
		if (
			(sc->data[SC_TRICKDEAD] && skill_id != NV_TRICKDEAD) || (sc->data[SC_AUTOCOUNTER] && !flag && skill_id) || (sc->data[SC_GOSPEL] && sc->data[SC_GOSPEL]->val4 == BCT_SELF && skill_id != PA_GOSPEL) || (sc->data[SC_SUHIDE] && skill_id != SU_HIDE))
			return false;
	...
	for (i = 0; i < EQI_MAX; i++)
	{
		...
		if (sd->inventory_data[index])
		{
			...
			{ // Uses MAX_SLOTS to support Soul Bound system [Inkfish]
				...
				if (!data)
					continue;
				if (opt & SCO_FIRST && data->equip_script && (pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(data, sd->bl.m)))
				{ // Execute equip-script on login
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, struct status_change *sc)
{
	...
	// No HP or SP regen
	if ((sc->data[SC_POISON] && !sc->data[SC_SLOWPOISON]) || (sc->data[SC_DPOISON] && !sc->data[SC_SLOWPOISON]) || sc->data[SC_BERSERK] || sc->data[SC_TRICKDEAD] || sc->data[SC_BLEEDING] || sc->data[SC_MAGICMUSHROOM] || sc->data[SC_SATURDAYNIGHTFEVER] || sc->data[SC_REBOUND])
		regen->flag = RGN_NONE;
...
void status_calc_state(struct block_list *bl, struct status_change *sc, enum scs_flag flag, bool start)
{
	...
	if (flag & SCS_NOMOVE)
	{
		if (!(flag & SCS_NOMOVECOND))
			...
			|| (sc->data[SC_GRAVITATION] && sc->data[SC_GRAVITATION]->val3 == BCT_SELF) || (sc->data[SC_CAMOUFLAGE] && sc->data[SC_CAMOUFLAGE]->val1 < 3))
			sc->cant.move += (start ? 1 : ((sc->cant.move) ? -1 : 0));
	}
...
int status_change_end_(struct block_list *bl, enum sc_type type, int tid, const char *file, int line)
{
	...
	case SC_CLOSECONFINE:
		if (sce->val2 > 0)
		{
			...
			int range = 1 + skill_get_range2(bl, status_sc2skill(type), sce->val1, true) + skill_get_range2(bl, TF_BACKSLIDING, 1, true); // Since most people use this to escape the hold....
	...
	case SC_SIGHTBLASTER:
			...
			status_check_skilluse(src, bl, WZ_SIGHTBLASTER, 2))
		{
			if (sce)
			{
				...
				if (skill_attack(BF_MAGIC, src, src, bl, WZ_SIGHTBLASTER, sce->val1, tick, 0x1000000) && (!su || !su->group || !(skill_get_inf2(su->group->skill_id) & INF2_TRAP)))
...
static bool status_yaml_readdb_refine_sub(const YAML::Node &node, int refine_info_index, const std::string &file_name)
{
	...
	if (file_name.find("import") != std::string::npos)
	{ // Import file, reset refine bonus before calculation
