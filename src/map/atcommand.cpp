...
ACMD_FUNC(param)
{
	...
	if (i == ARRAYLENGTH(param) || i > MAX_STATUS_TYPE)
	{												// normally impossible...
		clif_displaymessage(fd, msg_txt(sd, 1013)); // Please enter a valid value (usage: @str/@agi/@vit/@int/@dex/@luk <+/-adjustment>).
		return -1;
	}
...
ACMD_FUNC(char_ban)
{
	...
	if (timediff < 0 && ((bantype == CHRIF_OP_LOGIN_BAN && !pc_can_use_command(sd, "unban", COMMAND_ATCOMMAND)) || (bantype == CHRIF_OP_BAN && !pc_can_use_command(sd, "charunban", COMMAND_ATCOMMAND))))
	{
	...
	if ((sd->class_ & MAPID_THIRDMASK) == MAPID_RUNE_KNIGHT && pc_checkskill(sd, RK_DRAGONTRAINING) > 0)
	{
		if (!(sd->sc.option & OPTION_DRAGON))
		{
			...
			if (message[0])
			{
				...
				option = (color == 2 ? OPTION_DRAGON2 : color == 3 ? OPTION_DRAGON3 : color == 4 ? OPTION_DRAGON4 : color == 5 ? OPTION_DRAGON5 : OPTION_DRAGON1);
			}
...
ACMD_FUNC(gepard_block_nick)
{
	...
	violator_sd = map_nick2sd(violator_name, true);
